...
//Replace the user_id that has been deleted with string "deleted user":  //Not working yet
app.put("/database/new-users", async (req, res) => {
  const client = new MongoClient(uri);
        ...
        $set: {
          matches: {
            $map: {
              ...
              in: {
                $cond: {
                  if: { $eq: ["$$match.user_id", userId] },
                  then: { user_id: "deleted user" },
                  else: "$$match",
                },
              },
            },
